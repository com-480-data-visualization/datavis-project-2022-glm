		...
		d => {
			...
			function update_wrapper(data_, shift_val, y_axis_text) {
				...
				chart_selection
					.enter()
					.append("path")
					.attr("class", function(d) { return "myAreaNO NO" + d.key })
			...
			var full_range_button = d3.select("#full-range-no")
			var expanded_button = d3.select("#expanded-no")
...
whenDocumentLoaded(() => {
	plot_object = new StackedBasicAreaChartNO('stacked-basic-area-chart-no');
});
