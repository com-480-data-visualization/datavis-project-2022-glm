		...
		d => {
			...
			function update_wrapper(data_, shift_val, y_axis_text) {
				...
				chart_selection
					.enter()
					.append("path")
					.attr("class", function(d) { return "myAreaCH CH" + d.key })
			...
			var full_range_button = d3.select("#full-range-ch")
			var expanded_button = d3.select("#expanded-ch")
...
whenDocumentLoaded(() => {
	plot_object = new StackedBasicAreaChartCH('stacked-basic-area-chart-ch');
});
